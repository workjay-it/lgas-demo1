    ...
    if pin_input:
        if len(pin_input) == 6 and pin_input.isdigit():
            ...
            if not results.empty:
                ...
                st.caption(f"Overdue in this area: **{overdue_in_area}** ({overdue_in_area/len(results):.0%} if len(results) > 0 else 0})")
    ...
    if not df.empty:
        ...
        if st.button("Confirm Refill"):
            ...
            try:
                ...
                st.warning("Refill recorded in memory only (Streamlit Cloud is read-only).")
    else:
